...
项目开源在 GitHub（MIT 协议），面向 MiMo Orbit 百万亿 Token 创造者激励计划。
...
- **最大 8 轮**：每轮携带完整的工具定义和历史上下文，Token 消耗是普通对话的 5-10 倍
...
- **会话持久化**：所有对话历史保存为 JSON，包含 Token 记录，方便开发者评估"从 GPT 迁移到 MiMo"的成本效益
...
## 四、Token 消耗特征（与激励计划的关系）

| 场景 | 单次消耗 | 说明 |
|------|---------|------|
| Agent 多轮工具调用 | 8K-30K | 2-4 个工具链式调用，每次重发完整上下文 |
| 全项目代码审查 | 50K-200K | 多文件 @ 引用一次性加载 |
| 长上下文编程会话 | 50K-500K | 10+ Agent 轮次累积 |
| 多模态分析 | 5K-20K | 图片 base64 编码 + 文本 |

**Agent 模式下的 Token 消耗是普通对话的 5-10 倍**，这会为 MiMo Orbit 计划贡献显著且真实的 API 调用量。
